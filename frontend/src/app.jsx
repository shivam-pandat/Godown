...
function MainApp() {
  ...
  const load = async () => {
    try {
      ...
      console.error(e);
      // if token invalid, logout (Auth middleware will return 401)
  ...
  const handleScan = async (barcode) => {
    try {
      // you can prompt for change value. default -1 used here (outgoing)
      const change = -1;
      await scanBarcode({ barcode, change });
      await load();
      // quick notify
      // eslint-disable-next-line no-alert
      alert("Processed barcode: " + barcode);
    } catch (e) {
      // eslint-disable-next-line no-alert
  ...
  const visible = onlyLow
    ? items.filter((i) => i.low_threshold > 0 && i.quantity <= i.low_threshold)
    : items;
    ...
    <div className="p-6 bg-gray-100 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => checkLowStock().then(() => alert("Low stock check triggered"))}
        ...
        <div>
          ...
          <div className="mt-4">
            <label className="mr-2">
              <input type="checkbox" checked={onlyLow} onChange={(e) => setOnlyLow(e.target.checked)} /> Show only low-stock
            </label>
          </div>
          <ExportButtons />
          <div className="mt-4">
        ...
        <div className="md:col-span-2">
          {showScanner && (
            <div className="mb-4 p-2 bg-white rounded shadow">
              <BarcodeScannerWeb onDetected={handleScan} />
            </div>
          )}
